...
def main():
    ...
    command=[str(exe),'--background','--python-exit-code','86','--python',str(script)]
        ...
        while process.poll() is None:
            ...
            if time.monotonic()-start>args.timeout:
                ...
                try:process.wait(timeout=5)
                    ...
                    try:
                        attempt=subprocess.run(['taskkill','/PID',str(process.pid),'/T','/F'],capture_output=True,timeout=10)
                        result['termination_attempt']={'exit_code':attempt.returncode,'stderr':attempt.stderr.decode('utf-8',errors='replace')}
                    except subprocess.TimeoutExpired:
                        result['termination_attempt']={'exit_code':None,'timed_out':True}
                break
            time.sleep(.5)
    ...
    result['python_exception']=any(marker in (out/'stderr.log').read_text(encoding='utf-8',errors='replace') for marker in ('Traceback (most recent call last):','Error: Python:'))
    result['operation_completed']=result['exit_code']==0 and not result['timed_out'] and not result['python_exception']
