...
const watcher=setInterval(()=>{const now=performance.now();ticks.push(now-previous);previous=now;
 ...
 if(phase==='walk'){Object.assign(hero,{x:-110,z:-105,generation:hero.generation+1,direction:{x:0,z:0},target:null,autoAttack:false});world.motors.delete(id);world.checkpoint();}
...
let log='',native,error;const child=spawn(resolve(binaryArg),[...(project?['--path',resolve(project)]:[]),'--audio-driver','Dummy',...(graphical?['--windowed','--resolution','1600x900']:['--headless']),'--',`--bootstrap=${bridge.bootstrapPath}`,`--qa=${join(output,'hotfix-native.json')}`,'--qa-scope=hotfix',...(args.includes('--diagnose')?['--hotfix-diagnose']:[]),...(args.includes('--slow')?['--hotfix-slow']:[])],{env,windowsHide:true,stdio:['ignore','pipe','pipe']});
