...
const plan={schema:2,status:'approved-layout-art-map-preview-not-runtime',date:'2026-09-10',branch:'work/character-art-v3',baseline:'96dd25319eab2de99aba226b7ab07e0010214a8d',world:{oldWidth:320,oldDepth:280,width:1600,depth:1400,areaMultiplier:25,sideMultiplier:5,north:'+z',bounds:{minX:-800,maxX:800,minZ:-700,maxZ:700}},zones,routes,settlements,lake,cave,populations,bosses:[{id:'mini',center:[-430,260],radius:32},{id:'big',center:[-205,465],radius:40},{id:'rift_boss',center:[595,510],radius:48}],minimap:{northUp:true,refreshHz:10,viewMetres:[100,180,300],defaultViewMetres:180,caveViewMetres:100,hideUndetectedEnemies:true,source:'same detailed atlas, layer and coordinate transform as full map',buffOrder:'speed potion first; others by application order; row to the left of the top-right minimap'}};
// Approval applies to the existing layout/populations; this revision changes map presentation only.
plan.approval={date:'2026-09-10',layout:'approved',mapRequirement:'Full map and minimap must reproduce the approved world image',requiresAnotherLayoutApproval:false};
plan.cartography={preview:{surface:'world-concept.png',cave:'cave-map-art.png',metricAccuracy:'illustrative-concept-not-surveyed-terrain'},sharedAtlasForFullMapAndMinimap:true,sharedWorldToImageTransform:true,production:{source:'detailed orthographic bake of the actual built world per terrain layer',projection:'north-up, +z at top; identical bounds and version for both maps',detail:'terrain, vegetation, buildings, water, roads and entrances; no flat biome polygon replacement',excludeFromBackground:['characters','monsters','loot','weather','UI'],delivery:'tiled atlas with mip levels; update on geometry-version changes'},technicalDrawing:'world-plan.svg is an authoring reference, never the player-facing map background'};
...
// Technical authoring drawing only. Player-facing map previews use the shared art atlas.
...
const svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 1400" font-family="Segoe UI, sans-serif" role="img" aria-label="Техническая схема VARENDOR 1600 на 1400 метров"><title>VARENDOR: согласованный план мира</title><style>.compact-label{display:none}@media(max-width:700px){.full-label{display:none}.compact-label{display:block}.settlement-label{font-size:52px}.map-note{font-size:48px}}</style>${geometry}${labels}</svg>`;
...
const html=template.replace('/*__PLAN__*/',JSON.stringify(plan)).replace('/*__CANDIDATES__*/',JSON.stringify(candidates.map(c=>[c.id,c.p[0],c.p[1],c.layer]))).replace('/*__EVIDENCE__*/',JSON.stringify(evidence));
