...
if(options.includes('--stage')){
 ...
 writePackageManifest(stage,{...metadata,kind:'godot-content-v3-preview',qaScope:'content-v3',skillBooks:30,approvedMonsterModels:10});
 ...
 const manifest=read(join(stage,'PACKAGE_MANIFEST.json'));assert.equal(manifest.buildCommit,commit);assert.equal(manifest.qaScope,'content-v3');
...
`,stage,archive,epoch]);
 ...
 else{gh(['release','create',tag,...files,'--repo',repository,'--target',commit,'--draft','--prerelease','--latest=false','--title','Varendor — Books and monsters V3 Windows test','--notes-file',notes]);release=findRelease();}
