...
if(options.includes('--stage')){
 ...
 for(const [platform,folder]of [['linux','linux'],['win32','windows']]){
  const path=join(directory,`sliding-${folder}`,'hotfix-integration.json'),report=read(path);
  assert.equal(report.ok,true);assert.equal(report.source,commit);assert.equal(report.platform,platform);assert.equal(report.playerSavesOpened,false);
  assert.equal(report.native.ok,true);assert.ok(Object.keys(report.native.checks).length>=19);
  for(const [key,value]of Object.entries(report.native.checks))if(key!=='native_render')assert.equal(value,true,key);
  for(const metres of Object.values(report.native.stop_metres))assert.ok(metres<.015);
  if(platform==='linux')assert.equal(report.native.checks.native_render,true);
  if(platform==='win32')assert.equal(report.node,`v${read('packaging/windows/node-runtime.json').version}`);
  const target=join(output,`sliding-${folder}-native-qa.json`);cpSync(path,target);reports.push(target);
 }
...
`,stage,archive,epoch]);
 ...
 else{gh(['release','create',tag,...files,'--repo',repository,'--target',commit,'--draft','--prerelease','--latest=false','--title','Varendor — Teleport level and sliding hotfix','--notes-file',notes]);release=findRelease();}
