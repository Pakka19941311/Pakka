...
import {prepareNativeStabilityFixture} from './world_expansion_v3/native-stability-fixture.mjs';
...
try{
 ...
 hero.level=40;hero.gold=10000;world.recalculate(hero);hero.hp=hero.maxHp;hero.mp=hero.maxMp;world.checkpoint();
 ...
 fixtureTimer=setInterval(()=>{
  ...
  try{
   const result=prepareNativeStabilityFixture(world,geography,hero,request.stage);world.checkpoint();fixtureHistory.push(result);
 ...
 const summary={ok:exitCode===0&&report?.ok===true&&wrapper.clean_error_log&&sourceUnchanged,nativeExitCode:exitCode,cleanErrorLog:wrapper.clean_error_log,sourceCopyUsed:Boolean(sourceCopy),sourceCopyUnchanged:sourceUnchanged,
  fixtureHistory,checks:report?.checks??{},metrics:report?.metrics??{},reason:report?.reason??'Client did not produce final report',budgetSeconds:budget,initialTestHero:{class:'knight',level:40,gold:10000},population:world.state.monsters.length};
