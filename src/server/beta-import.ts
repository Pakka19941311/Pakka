...
export function parseBetaSave(raw: unknown) {
  ...
  const item = (rawItem: unknown): InventoryItem => {
    ...
    return {uid:i.uid,id:i.id,plus,count};
  };
  ...
  for (const [slot,value] of Object.entries(record(player.equipment))) {
    ...
    if (!definition || !('slot' in definition) || !compatibleEquipmentSlots(definition.slot).includes(slot)) return invalid();
  ...
  return {
    ...
    dead:Boolean(player.dead || hp <= 0),inventory:migrated.player.inventory,equipment,lootBuffer:migrated.lootBuffer,
