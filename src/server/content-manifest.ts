...
export const CONTENT_VERSION='catalog-v1-'+digest({CLASSES,ITEMS,MONSTERS,SCROLLS,ENHANCEMENT_PERCENT});
export function mapVersion(collision:CollisionWorld,terrain:TerrainSurface):string {
  return 'terrain-'+TERRAIN_VERSION+'-'+digest({colliders:collision.manifest(),platforms:terrain.platformManifest(),heights:Array.from(terrain.heights)});
}
