...
function useItem(id: string, reference?: ItemReference): void {
  ...
  if (id === 'potion' || id === 'potion_large') { if (player.hp >= player.maxHp) return toast('Здоровье уже полное'); if (!consumeItem(id, reference)) return toast('Нет багровых зелий', 'bad'); player.hp = Math.min(player.maxHp, player.hp + (id === 'potion' ? 37 : 70)); gameAudio.play('potion', 0.74, 0.92); toast('Здоровье восстановлено'); log('Багровое зелье: здоровье восстановлено.', 'combat'); }
