...
export function quickDefaults(): QuickSlot[] {
  return Array.from({length: 32}, (_, i) => ({action: i === 4 ? 'attack' : i === 8 ? 'potion' : i === 9 ? 'ether' : i === 10 ? 'teleport' : '', key: i < 16 ? `${i >= 8 ? 'Shift+' : ''}Digit${i % 8 + 1}` : ''}));
}
export function normalizeQuickbar(value: unknown): QuickSlot[] {
  ...
  return defaults.map((fallback, i) => {
    ...
    const action = ['', 'attack', 'potion', 'ether', 'teleport'].includes(entry?.action) ? entry.action : fallback.action;
