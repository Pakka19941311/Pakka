...
func run() -> void:
	...
	for index: int in range(-8, 9):
		...
		stripe.size = Vector3(6, .001, .012)
		marker.mesh = stripe
		marker.position = Vector3(0, 0, float(index) * .25)
		var stripe_mat := StandardMaterial3D.new()
		stripe_mat.albedo_color = Color("73756f")
		marker.material_override = stripe_mat
	...
	var profiles: Dictionary = {"MOB-04":ADAPTER.profiles()["MOB-04"]}
	for mob: String in profiles:
		...
		for frame: int in range(60):
			actor.position.z = -1.6 * float(frame) / 60.0
			camera.position = center + Vector3(1.2, .8, -1.8) * size + actor.position
			camera.look_at(center + actor.position, Vector3.UP)
			controller.update(alive, Vector3(0,0,-1.6), 1100+frame*1000.0/60.0, 1.0/60.0)
			if frame in [0, 10, 20, 30, 40, 50]:
				title.text = "MOB-04 · Boar · pursuit1.6m/s · phase " + str(frame)
				await capture("MOB-04_run_"+str(frame))
		...
		var impact: float = 2000 + duration * float(profile.qa_server_contact_fraction)
	...
	var file := FileAccess.open(output.path_join("BOAR_NATIVE_QA.json"), FileAccess.WRITE)
