...
func run_review() -> void:
	details=load("res://world-final/nature/groundcover_layer.gd").new();add_child(details)
	await details.build()
	var cliff_obstacles: Array=JSON.parse_string(FileAccess.get_file_as_string("res://world-final/nature/groundcover-collision-D11.json")).obstacles
	...
	if FileAccess.file_exists(report_path):
		...
		report["revision"]="D11";report["groundcover_counts"]=details.data.counts;report["native_source"]=details.data.native_source
