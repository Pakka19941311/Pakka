...
func _draw() -> void:
	if artwork != null:
		draw_texture_rect(artwork, Rect2(Vector2((size.x - 43) / 2, 8), Vector2(43, 43)), false, Color.WHITE if usable else Color("737773"))
	if remaining > 0:
	...
	draw_string_outline(font, Vector2(5, 14), key_label, HORIZONTAL_ALIGNMENT_LEFT, -1, 11, 4, Color("101618"))
	draw_string(font, Vector2(5, 14), key_label, HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color("d2c6a6"))
