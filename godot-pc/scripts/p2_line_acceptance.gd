...
static func run(app: Node,checks: Dictionary) -> void:
	...
	for stage: String in ["p2-line-fire","p2-line-ice-dodge"]:
		...
		if not checks[prefix+"fixture"]:
			app.world.event_presented.disconnect(listener);continue
		...
		await Wait.capture(app,stage+"-warm")
		...
		await app.net.intent({"type":"destination","x":fixture.engagePoint.x,"z":fixture.engagePoint.z})
		timings.approach_ack=Time.get_ticks_msec()
		...
		await app.net.intent({"type":"attack","entityId":fixture.targetUid,"mode":"single","skill":null})
		timings.attack_ack=Time.get_ticks_msec()
		...
		if not shown:
			RenderingServer.frame_post_draw.disconnect(draw_observer);app.world.event_presented.disconnect(listener);continue
		var id: String=""
		...
		if stage.ends_with("dodge"):
			await app.net.intent({"type":"destination","x":fixture.sidestep.x,"z":fixture.sidestep.z})
		else:await app.net.intent({"type":"cancel"})
		timings.response_ack=Time.get_ticks_msec()
		...
		report.append({"fixture":fixture,"timings":timings,"events":events.filter(func(e):return e.get("actor","")==fixture.targetUid),"frames":frames,"frame_trace":frame_trace,"draws":observation.draws,"initial_hp":initial_hp,"final_hp":final_hp,"moved":moved,"pulse_seen":observation.pulse_seen,"pulse_count":observation.pulse_ids.size()})
