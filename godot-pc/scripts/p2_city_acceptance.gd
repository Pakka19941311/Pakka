...
static func run(app: Node, checks: Dictionary) -> void:
	...
	checks.p2_city_street_capture = await Capture.capture(app,"p2-city-street")
	...
	checks.p2_city_stair_top_capture = await Capture.capture(app,"p2-city-stair-top")
	...
	checks.p2_city_stair_clear_view_capture = await Capture.capture(app,"p2-city-stair-clear-view")
	...
	checks.p2_city_stair_bottom_capture = await Capture.capture(app,"p2-city-stair-bottom")
