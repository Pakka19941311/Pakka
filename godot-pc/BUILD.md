...
P1-11: `scripts/godot-pc-samples.py` создаёт шесть собственных образцов: стену из четырёх стыкуемых модулей, угол, арку, валун, сосну с тремя LOD и траву. Сохраняются исходный .blend, GLB, размеры, seed, colliders, источники материалов. Исходные PBR-карты сохраняют прежние лицензии. `art_review.gd` проверяет масштаб с прежней моделью героя, проходимость и три ракурса каждого образца; ветви и трава имеют маску ветра, древесина неподвижна. Графическая приёмка владельца ещё требуется перед P1-12 — автоматические кадры её не заменяют.

Для чистой сборки после основной Blender-сцены также выполнить `blender --background --factory-startup --threads 2 --python-exit-code 1 --python scripts/godot-pc-samples.py`. В пакет входят VIEW_WORLD_SAMPLES.bat и исходники world-sources/P1.
