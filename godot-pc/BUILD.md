...
HTTP v1: session, world polling 10 Hz, input, command и disconnect. Polling выбран для первого PC-теста; SSE parser/расширенный epoch/contentVersion и промышленный reconnect остаются открытыми подпунктами P1. Старый snapshot не откатывает новый revision. Предметная команда записывается перед отправкой; retry использует тот же commandId и payload, результат показывает серверная receipt. Сравнение и equip используют одинаковый порядок ring1/ring2, ear1/ear2.
