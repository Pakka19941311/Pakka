...
**Статус: план мира согласован 10 сентября 2026 года.** Последнее уточнение владельца: «вид миникарты дублировал вид образа мира так же и с обычной картой. Все остальное согласовано». Полная карта и миникарта должны сохранять подробный художественный вид принятого мира. Размеры, локации, популяции и порядок производства утверждены; повторное согласование этой схемы не требуется. Этот пакет содержит дизайн и макет карт; игровая территория ещё не перестроена. Рабочая ветка — `work/character-art-v3`, исходная точка плана — `96dd25319eab2de99aba226b7ab07e0010214a8d`.
...
- Карта и локации согласованы. Далее последовательно создаются геометрия, окружение, спавны, навигация, полная карта и миникарта. После этого возвращаемся к очереди из [коммерческого анализа](../analysis-v3/R2_COMMERCIAL_GAP_ANALYSIS_RU.md).
...
![Технический чертёж для построения мира. Цветные полигоны не используются как фон игровых карт.](world-plan.svg)

Открываемый локально [обзор карты](WORLD_REVIEW.html) показывает художественную карту мира, распределение монстров поверх неё, чистый образ без подписей и художественный атлас пещеры. Щелчок по большой карте или выбор локации меняет участок миникарты; светлая рамка показывает границы этого участка. Ночь и полнолуние показывают соответствующие популяции. GitHub отображает HTML как исходник; изображения доступны прямо в документе. SVG выше сохранён исключительно как технический чертёж.
...
- **Обязательное уточнение владельца:** обычная карта и миникарта повторяют подробный художественный образ мира. На них читаются кроны, скалы, снег, вода, дороги, здания и кладбище. Цветные полигоны, абстрактные пятна биомов и отдельная упрощённая перерисовка миникарты не заменяют этот фон.
- **Один атлас на слой.** Полная карта показывает весь атлас, миникарта кадрирует и масштабирует его же. Одна система координат, ориентация и версия. Маркеры и подписи — отдельный слой поверх изображения, без изменения самого фона.
- **Макет и игровой источник.** Сейчас поверхность обеих карт использует неизменённый `world-concept.png`, пещера — `cave-map-art.png`. Это художественные иллюстрации, ещё не точная привязка к игровому рельефу. При производстве фон запекается ортографически сверху с реальной построенной сцены по каждому слою, сохраняя материалы и выразительность принятого образа. Наклонная перспектива концепта не используется для вычисления игровых координат. Из запекания исключаются персонажи, монстры, добыча, погода и UI; они не остаются нарисованными в пустых местах карты.
...
- Берег, реальные дороги, ворота и перекрывающие движение объекты совпадают с игровой сценой. Координаты переводятся через общие границы слоя: `u = (x − minX) / (maxX − minX)`, `v = (maxZ − z) / (maxZ − minZ)`. Для поверхности границы 1600×1400 м, для пещеры 360×320 м. Перспективное смещение картинки не компенсируется ручным сдвигом маркеров.
...
- Фон карты запекается/кэшируется по территории и слою, а не пересобирается каждый кадр. Для 1600×1400 м используется тайловый атлас с уровнями детализации; на обзоре 100 м должны различаться дорожки, берега и входы без растянутого размытого общего изображения. Движущиеся маркеры обновляются до 10 раз в секунду. Для новой геометрии требуется новый ключ версии кэша, общий для обеих карт.
- Приёмка: сравнить в игре большие и малые карты у ворот двух городов, обоих мостов, входа/выхода пещеры, серпантинов, берега, кладбища и арены вулкана. Уменьшение/увеличение масштаба и смена слоя не дают скачка маркера, зеркальности, сдвига зданий или обрезанного прохода. HUD и порядок бафов сохраняются. Макет проверяет общий фон и кадрирование; совпадение с 3D-сценой проверяется после её построения.
...
## 8. Согласованная последовательность реализации
...
Художественный атлас пещеры: [cave-map-art.png](cave-map-art.png), исходный промпт и назначение — [CAVE_MAP_PROMPT.md](CAVE_MAP_PROMPT.md). HTML открывается вместе с обоими PNG из того же каталога. Внешних запросов для карт нет.
