...
## Актуальное решение 2026-09-06

Постоянный серверный мир разрешён владельцем и включён в первую часть нового плана `REBUILD_THREE_PARTS_RU.md`. Это замещает прежнее откладывание сервера. Исходный HEAD `1e94a0d1ad08a4df8959527e40058f5c89b07e9a` ещё использует `LocalGameGateway` и симуляцию в браузере; наличие этого документа не подтверждает сетевую реализацию.

Сдачи: мир → герои/NPC → монстры, с остановкой на пользовательский тест после каждой. Технические проверки ограничиваются изменениями части и непосредственно затронутыми контрактами, а не полной старой матрицей игры.
...
- Камера, сбор input, явный targeting и combat pursuit разделены в `src/controls`; UI только отображает состояние и отправляет пользовательские команды.
- Ручное движение отправляет нормализованные последовательные `move-intent` команды через `GameGateway`, чтобы локальную симуляцию позднее можно было заменить server-authoritative обработкой без переписи input/UI.
