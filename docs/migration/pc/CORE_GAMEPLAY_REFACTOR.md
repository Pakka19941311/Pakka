...
**Финальный статус сборки заполняется после завершения CI:**

| Проверка | Результат |
| --- | --- |
| Commit исходников итоговой сборки | [`cc8773404bb5`](https://github.com/Pakka19941311/Pakka/commit/cc8773404bb585adbe628c358564facb303ce7d9) |
| GitHub Actions: ссылка и результат | [Первый запуск 34163878654](https://github.com/Pakka19941311/Pakka/actions/runs/34163878654): две проверки курсора при модальном окне не прошли; исправлено, повтор ожидается |
...
| Linux Godot с графикой, X11/Mesa; 20 захватов мыши | Ожидается итоговый отчёт |
| Четыре отрисованных стадии боя: подготовка, полёт, попадание, труп | Ожидается просмотр реальных кадров |
| Windows EXE и сервер из поставляемого пакета | Ожидается итоговый отчёт; headless запуск не подтверждает Windows GUI |
| ZIP: URL, размер, SHA-256, проверка скачивания | Ожидается публикация и проверка |
| Приёмка управления и графики владельцем на игровом Windows-ПК | Требуется игровой тест |

До заполнения этих строк документ является отчётом об изменениях и имеющихся доказательствах, а не заявлением, что все 44 условия приёмки уже выполнены.
...
Обозначения: **S** — подтверждено серверными автоматическими тестами; **H** — подтверждено состояниями в успешном headless Godot runtime; **G** — необходим запуск с графикой и системной мышью; **M** — ручная проверка ощущения и изображения на игровом ПК. Наличие S/H не превращает G/M в выполненную проверку.

| № | Условие владельца | Автоматическое доказательство и остающаяся проверка |
| --- | --- | --- |
| 1 | ПКМ нажать/отпустить 20 раз | G: `rmb_20_os_cursor_restores`; headless не способен подтвердить ОС-курсор |
...
| 41 | NPC в городе минимум 60 секунд | H: реальное непрерывное время live-клиента; успешный повтор 60.0 с; G: повтор с графикой |
...
Автотест системного курсора под Linux/X11 подтвердит этот backend. Он не доказывает Windows-мышь при другом DPI, fullscreen, двух мониторах или драйвере. Показатели headless/программного Mesa не являются FPS на игровом Windows-ПК.
...
5. Общая приёмка всех 44 требований и субъективная отзывчивость остаются открытыми до результатов G/M из таблицы. Отсутствие GUI-проверки нельзя заменить заявлением об исправленном визуальном баге.
